...
num_symt = [150 300 500]; % number of symbols of training mode (tm)
...
mu = 0.001; % given step factor

for j = 1:length(num_symt)

num_ite = num_symt(j) + num_symd; % number of total iterations
   
%generating the data signal for training (4QAM) and
%control-by-decision(16QAM)
data_t = randi(const_sizet, num_symt(j), 1) - 1; 
...
% part II: control by decision mode. 
    ...
    for k = num_symt(j)+1:num_symt(j)+num_symd % É nessessario esperar um momento (amostras) até fazer a comparação com o sinal (Seção 2.10.4 pag.57 Diniz) . Nesse caso 7 ou 8(metade do comprimento do filtro), pois tem comprimento 15.
        inp = [x(k); init];
        init = inp(1:end-1);
...
figure(3*j-1)
plot3(real(eq_out(num_symt(j)+1:end)),imag(eq_out(num_symt(j)+1:end)),1:num_symd,'r.'); % plota da modulação 16 QAM
figure(3*j-2)
